...
function doGet(e) {
  try {
    ...
    // === NEW: Tarik semua attempts untuk guru (rekap) ===
    if (action === "getAttempts" || action === "getAllAttempts" || action === "getattempts") {
      const sheet = getOrCreateSheet(SHEET_NAMES.ATTEMPTS, ["timestamp","attemptId","studentName","kelas","quizId","quizJudul","score","correct","total","date","answersJson"]);
      const rows = sheet.getDataRange().getValues();
      let attempts = [];
      for (let i = 1; i < rows.length; i++) {
        try {
          attempts.push({
            ...
            kelas: rows[i][3] || "",
            studentClass: rows[i][3] || "",
            quizId: rows[i][4] || "",
            quizJudul: rows[i][5] || "",
            quizTitle: rows[i][5] || "",
            score: parseInt(rows[i][6]||0,10),
            correct: parseInt(rows[i][7]||0,10),
            total: parseInt(rows[i][8]||0,10),
            totalQuestions: parseInt(rows[i][8]||0,10),
            date: rows[i][9] || rows[i][0] || "",
            completedAt: rows[i][9] || rows[i][0] || "",
            createdAt: rows[i][0] || "",
            answers: (()=>{try{return JSON.parse(rows[i][10]||"[]")}catch{return []}})(),
            timestamp: rows[i][0] || ""
          });
        } catch(e) {}
      }
      // Optional filter by kelas if provided
      let filterKelas = (data.kelas || data.filterKelas || "").toString().trim();
      if(filterKelas){
        attempts = attempts.filter(a => (a.kelas||"").toLowerCase() === filterKelas.toLowerCase());
      }
      return jsonResponse({status:"ok", count: attempts.length, attempts: attempts, serverDate: serverToday});
    }

    return jsonResponse({status:"ok", message:"V7.1 Anti-Farming + Import Excel + getAllAttempts Active - Server date: "+serverToday, time: new Date().toISOString(), serverDate: serverToday});
  } catch (err) {
    return jsonResponse({status:"error", message: err.toString()});
  }
}
